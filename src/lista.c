 ...
 */
nodo_t* cargar_nodo(nodo_t* nodo_a_cargar, void* elemento, char* clave){
    nodo_t* nuevo = crear_nodo();
    if(!nuevo) return NULL;
    
    nodo_a_cargar = nuevo;
    nodo_a_cargar->elemento = elemento;
    nodo_a_cargar->clave = clave;
...
lista_t* lista_insertar(lista_t* lista, void* elemento, char* clave){
    ...
    if(!lista->nodo_inicio){
        ...
        lista->nodo_inicio->clave = clave;
        lista->nodo_fin = lista->nodo_inicio;
    }
    else{
        lista->nodo_fin->siguiente = cargar_nodo(lista->nodo_fin->siguiente, elemento, clave);
 ...
 */
lista_t* lista_insertar_en_posicion(lista_t* lista, void* elemento, char* clave, size_t posicion){
    if(!lista) return NULL;
    ...
    if(posicion >= (lista->cantidad)){
        lista_insertar(lista, elemento, clave);
        return lista;
    }
    else if(posicion == 0){
        nuevo = cargar_nodo(nuevo, elemento, clave);
        ...
        nuevo_elemento = cargar_nodo(nuevo_elemento, elemento, clave);
